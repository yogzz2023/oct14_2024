...
def main():
    file_path = 'ttk_31.csv'
    log_file_path = 'detailed_log.csv'
    
    # Initialize CSV log file
    with open(log_file_path, 'w', newline='') as csvfile:
        fieldnames = ['Time', 'Measurement MR', 'Measurement MA', 'Measurement ME', 'Current State',
                      'Correlation Window Threshold', 'Correlation Output', 'Associated Track ID',
                      'Associated Position MR', 'Associated Position MA', 'Associated Position ME',
                      'Association Type', 'Clusters Formed', 'Hypotheses Generated', 'Probability of Hypothesis',
        ...
        if len(group) == 1:  # Single measurement
            ...
            for track_id, track in enumerate(tracks):
                if correlation_check(track, measurement, doppler_threshold, range_threshold):
                    ...
                    log_data = {
                        'Time': measurement[3],
                        'Measurement MR': measurement[0],
                        'Measurement MA': measurement[1],
                        'Measurement ME': measurement[2],
                        'Current State': current_state,
                        'Correlation Window Threshold': kalman_filter.gate_threshold,
                        'Correlation Output': 'Yes',
                        'Associated Track ID': track_id,
                        'Associated Position MR': track['Sf'][0, 0],
                        'Associated Position MA': track['Sf'][1, 0],
                        'Associated Position ME': track['Sf'][2, 0],
            ...
            if not assigned:
                ...
                log_data = {
                    ...
                    'Associated Position MR': '',
                    'Associated Position MA': '',
                    'Associated Position ME': '',
            ...
            if association_method == 'JPDA':
                clusters = form_clusters_via_association([track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter, kalman_filter.gate_threshold)
            ...
            for track_id, best_report in best_reports:
                ...
                log_data = {
                    'Time': group[0][3],
                    'Measurement MR': best_report[0],
                    'Measurement MA': best_report[1],
                    'Measurement ME': best_report[2],
                    'Current State': current_state,
                    'Correlation Window Threshold': kalman_filter.gate_threshold,
                    'Correlation Output': 'Yes',
                    'Associated Track ID': track_id,
                    'Associated Position MR': tracks[track_id]['Sf'][0, 0],
                    'Associated Position MA': tracks[track_id]['Sf'][1, 0],
                    'Associated Position ME': tracks[track_id]['Sf'][2, 0],
            ...
            for report in reports:
                if tuple(report) not in assigned_reports:
                    ...
                    log_data = {
                        'Time': group[0][3],
                        'Measurement MR': report[0],
                        'Measurement MA': report[1],
                        'Measurement ME': report[2],
                        'Current State': 'Poss1',
                        'Correlation Window Threshold': kalman_filter.gate_threshold,
                        'Correlation Output': 'No',
                        'Associated Track ID': new_track_id,
