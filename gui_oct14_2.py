...
def plot_measurements(tracks, canvas):
...
def main(input_file, track_mode, filter_option, association_type, output_widget):
    ...
    for group_idx, group in enumerate(measurement_groups):
        output_widget.append(f"Processing measurement group {group_idx + 1}...")
        ...
        if current_time - last_check_time >= check_interval:
            ...
            for track_id in reversed(tracks_to_remove):
                output_widget.append(f"Removing track {track_id} due to timeout")
                del tracks[track_id]
    ...
    csv_data = []
    for track_id, track in enumerate(tracks):
        output_widget.append(f"Track {track_id}:")
        output_widget.append(f"  Current State: {track['current_state']}")
        output_widget.append(f"  State Transition Times:")
        for state, time in state_transition_times.get(track_id, {}).items():
            output_widget.append(f"    {state}: {time}")
        output_widget.append("  Measurement History:")
        for state in ['Poss1', 'Tentative1', 'Firm']:
            measurements = [m for m, s in track['measurements'] if s == state][:3]
            output_widget.append(f"    {state}: {measurements}")
        output_widget.append(f"  Track Status: {track_id_list[track_id]['state']}")
        output_widget.append(f"  SF: {track['Sf']}")
        output_widget.append(f"  SP: {track['Sp']}")
        output_widget.append(f"  PF: {track['Pf']}")
        output_widget.append(f"  PP: {track['Pp']}")
    ...
    output_widget.append(f"Track summary has been written to {csv_file_path}")
...
class KalmanFilterGUI(QWidget):
    ...
    def initUI(self):
        ...
        layout.addWidget(self.track_mode_label)
        layout.addWidget(self.track_mode_combo)
        ...
        layout.addWidget(self.filter_option_label)
        ...
        layout.addWidget(self.association_type_label)
        layout.addWidget(self.association_type_combo)
    ...
    def process_data(self):
        ...
        if not input_file:
            self.output_display.append("Please select an input file.")
            return

        self.output_display.append(f"Processing with:\nInput File: {input_file}\nTrack Mode: {track_mode}\nFilter Option: {filter_option}\nAssociation Type: {association_type}")

        # Call the main processing function with the selected options
        main(input_file, track_mode, filter_option, association_type, self.output_display)

    def show_plot(self):
        # Assuming `tracks` is available globally or passed to this function
        plot_measurements(tracks, self.figure_canvas)

if __name__ == '__main__':
    app = QApplication(sys.argv)
    window = KalmanFilterGUI()
    window.show()
    sys.exit(app.exec_())
