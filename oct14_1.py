...
def perform_munkres(tracks, reports, kalman_filter):
    ...
    clusters = [(row_ind, col_ind)]  # Example of forming clusters
    return best_reports, clusters
...
def main():
    ...
    firm_threshold = 3
    mode = '3-state'
    association_method = 'Munkres'  # Change this to 'Munkres' to use the Munkres algorithm
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        if len(group) == 1:  # Single measurement
            ...
            if association_method == 'JPDA':
                best_reports, clusters = perform_jpda([track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter)
            elif association_method == 'Munkres':
                best_reports, clusters = perform_munkres([track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter)
